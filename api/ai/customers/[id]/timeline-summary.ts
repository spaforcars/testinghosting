...
import { buildCustomerTimelineSummarySuggestion } from '../../../_lib/ai';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const suggestion = await buildCustomerTimelineSummarySuggestion(supabase, clientId, auth.userId);

    await writeAuditLog(supabase, {
      userId: auth.userId,
      action: 'generate_customer_timeline_summary',
      module: 'clients',
      entityType: 'customer',
      entityId: clientId,
      details: {
