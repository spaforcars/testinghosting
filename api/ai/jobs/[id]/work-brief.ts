...
import { buildJobWorkBriefSuggestion } from '../../../_lib/ai';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const suggestion = await buildJobWorkBriefSuggestion(supabase, jobId, auth.userId);

    await writeAuditLog(supabase, {
      userId: auth.userId,
      action: 'generate_ai_work_brief',
      module: 'services',
